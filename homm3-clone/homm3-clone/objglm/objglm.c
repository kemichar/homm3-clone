...
glmSecondPass(GLMmodel* model, FILE* file) 
{
  ...
  __glmWarning(" Memory: %d bytes",
	...
	// store indices to use with VBO
	GLMtriangle* triangle;
	GLMmaterial* tempMat;
	for (GLMgroup* group = model->groups; group; group = group->next) {
		tempMat = &model->materials[group->material];
		for (int i = 0; i < group->numtriangles; i++) {
			triangle = &T(group->triangles[i]);

			for (int j = 0; j < 3; j++) {
				model->indices[model->numindices] = triangle->vindices[j];
				model->diffuse[4 * model->numindices] = tempMat->diffuse[0];
				model->diffuse[4 * model->numindices + 1] = tempMat->diffuse[1];
				model->diffuse[4 * model->numindices + 2] = tempMat->diffuse[2];
				model->diffuse[4 * model->numindices + 3] = tempMat->diffuse[3];
				model->numindices++;
			}
			
		}
		printf("%d: %f %f %f\n", group->material, tempMat->diffuse[0], tempMat->diffuse[1], tempMat->diffuse[2]);
	}
}
...
glmDelete(GLMmodel* model)
{
	...
	if (model->diffuse) free(model->diffuse);
    if (model->materials) {
        for (i = 0; i < model->nummaterials; i++)
	    {
...
glmReadOBJ(const char* filename)
{
    ...
    model->triangles = (GLMtriangle*)malloc(sizeof(GLMtriangle) *
					    model->numtriangles);
	model->indices = (GLuint*)malloc(sizeof(GLuint) *
						3 * model->numtriangles);
	model->diffuse =  (GLfloat*)malloc(sizeof(GLfloat) *
				       4 * 3 * model->numtriangles);
    if (model->numnormals) {
...
void initModel(GLMmodel* model) {
	glGenBuffers(1, &model->vertexVBO);
	glBindBuffer(GL_ARRAY_BUFFER, model->vertexVBO);
	glBufferData(GL_ARRAY_BUFFER, model->numvertices * sizeof(GLfloat) * 3, &model->vertices[0], GL_STATIC_DRAW);
	glVertexPointer(3, GL_FLOAT, sizeof(GLfloat) * 3, 0);

	glGenBuffers(1, &model->normalVBO);
	glBindBuffer(GL_ARRAY_BUFFER, model->normalVBO);
	glBufferData(GL_ARRAY_BUFFER, model->numvertices * sizeof(GLfloat), &model->normals[0], GL_STATIC_DRAW);
	glNormalPointer(GL_FLOAT, sizeof(GLfloat), 0);

	glGenBuffers(1, &model->diffuseVBO);
	glBindBuffer(GL_ARRAY_BUFFER, model->diffuseVBO);
	glBufferData(GL_ARRAY_BUFFER, model->numvertices * sizeof(GLfloat) * 4, &model->diffuse[0], GL_STATIC_DRAW);
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, model->numindices * sizeof(GLuint), &model->indices[0], GL_STATIC_DRAW);
